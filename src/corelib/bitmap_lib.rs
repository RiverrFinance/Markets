...
mod unit_test {
    ...
    mod buy_test {
        ...
        fn test_next_initialised_tick() {
            ...
            let bitmap2 = 11; // ......01011

            let next2 = _next_initialised_tick(bitmap2, integral, 95, true);
            assert_eq!(next2, (integral * _ONE_PERCENT) + (96 * _ONE_BASIS_POINT));

            let next2 = _next_initialised_tick(bitmap2, integral, 96, true);

            assert_eq!(next2, (integral * _ONE_PERCENT) + (98 * _ONE_BASIS_POINT));
        }
    }
    ...
    mod sell_test {
        ...
        #[test]
        fn test_next_initialised_for_sell() {
            let bitmap1 = 0;
            let integral = 100;
            let next1 = _next_initialised_tick(bitmap1, integral, 0, false);
            assert_eq!(
                next1,
                (integral - 1) * _ONE_PERCENT + (99 * _ONE_BASIS_POINT)
            );

            let next2 = _next_initialised_tick(bitmap1, integral, 1, false);
            assert_eq!(next2, (integral) * _ONE_PERCENT);

            //
            let bitmap2 = 11; // ......1011

            let next3 = _next_initialised_tick(bitmap2, integral, 99, false);
            assert_eq!(next3, ((integral) * _ONE_PERCENT) + (98 * _ONE_BASIS_POINT));

            // comtinuing from the previous test;
            let next3 = _next_initialised_tick(bitmap2, integral, 98, false);

            assert_eq!(next3, ((integral) * _ONE_PERCENT) + (96 * _ONE_BASIS_POINT));
        }
    }
}
